...
class movieRepository(object):
    ...
    def add(self, movie):
        if movie.getId() in self.movies:
            ...
            raise RepositoryException(["Movie with ID " + str(movie.getId()) + " already exists."])
    ...
    def update(self, movie):
        if not movie.getId() in self.movies:
            ...
            raise RepositoryException(["Can't update movie with ID " + str(movie.getId()) + " because it doesn't exist."])
    ...
    def remove(self, ID):
        if not ID in self.movies:
            ...
            raise RepositoryException(["Can't remove movie with ID " + str(ID) + " because doesn't exist."])
